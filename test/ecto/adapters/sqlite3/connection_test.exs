defmodule Ecto.Adapters.SQLite3.ConnectionTest do
  ...
  defp plan(query, operation \\ :all) do
    {query, _cast_params, _dump_params} =
      Ecto.Adapter.Queryable.plan_query(operation, Ecto.Adapters.SQLite3, query)
  ...
  test "arrays and sigils" do
    ...
    assert all(query) ==
             ~s{SELECT s0."w" FROM "schema" AS s0 WHERE (s0."w" = JSON_ARRAY('[]'))}
  end
  ...
  test "update all array ops" do
    ...
    assert update_all(query) ==
             ~s{UPDATE "schema" AS s0 SET "w" = JSON_INSERT("w",'$[#]',0)}
