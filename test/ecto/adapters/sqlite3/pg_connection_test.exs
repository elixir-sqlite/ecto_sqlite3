defmodule Ecto.Adapters.SQLite3Test do
  ...
  defmodule Schema do
    ...
    schema "schema" do
      ...
      has_many :comments, Ecto.Adapters.SQLite3Test.Schema2,
        references: :x,
        foreign_key: :z
      has_one :permalink, Ecto.Adapters.SQLite3Test.Schema3,
        references: :y,
        foreign_key: :id
    end
  end
  ...
  defmodule Schema2 do
    ...
    schema "schema2" do
      belongs_to :post, Ecto.Adapters.SQLite3Test.Schema,
        references: :x,
        foreign_key: :z
    end
  end
  ...
  defp plan(query, operation \\ :all) do
    {query, _cast_params, _dump_params} = Ecto.Adapter.Queryable.plan_query(operation, Ecto.Adapters.SQLite3, query)
    query
  end
  ...
  test "parent binding subquery and CTE" do
    ...
    assert_raise Ecto.QueryError, ~r/join `:left_lateral` not supported by SQLite3/, fn ->
      all(query)
    end
  end
  ...
  test "lock" do
    ...
    assert_raise ArgumentError, "locks are not supported by SQLite3", fn ->
      Schema |> lock([p], fragment("UPDATE on ?", p)) |> select([], true) |> plan() |> all()
    end
  ...
  test "datetime_add" do
    ...
    assert all(query) == ~s{SELECT 1 FROM "schema" AS s0 WHERE (datetime(s0."foo",'1 month') > s0."bar")}

    query = "schema" |> where([s], datetime_add(type(s.foo, :string), 1, "month") > s.bar) |> select([], true) |> plan()
    assert all(query) == ~s{SELECT 1 FROM "schema" AS s0 WHERE (datetime(CAST(s0."foo" AS TEXT),'1 month') > s0."bar")}
  end
  ...
  test "optimized json_extract_path" do
    ...
    assert all(query) == ~s|SELECT 1 FROM "schema" AS s0 WHERE (json_extract(s0.\"meta\", '$.id') = 123)|

    query = Schema |> where([s], s.meta["id"] == "123") |> select(true) |> plan()
    assert all(query) == ~s|SELECT 1 FROM "schema" AS s0 WHERE (json_extract(s0."meta", '$.id') = '123')|

    query = Schema |> where([s], s.meta["tags"][0]["name"] == "123") |> select(true) |> plan()
    assert all(query) == ~s|SELECT 1 FROM "schema" AS s0 WHERE (json_extract(s0."meta", '$.tags[0].name') = '123')|

    query = Schema |> where([s], s.meta[0] == "123") |> select(true) |> plan()
    assert all(query) == ~s|SELECT 1 FROM "schema" AS s0 WHERE (json_extract(s0.\"meta\", '$[0]') = '123')|

    query = Schema |> where([s], s.meta["enabled"] == true) |> select(true) |> plan()
    assert all(query) == ~s|SELECT 1 FROM "schema" AS s0 WHERE (json_extract(s0."meta", '$.enabled') = 1)|

    query = Schema |> where([s], s.meta["extra"][0]["enabled"] == false) |> select(true) |> plan()
    assert all(query) == ~s|SELECT 1 FROM "schema" AS s0 WHERE (json_extract(s0."meta", '$.extra[0].enabled') = 0)|
  end
  ...
  test "order_by and types" do
    ...
    assert all(query) == "SELECT 1 FROM \"schema3\" AS s0 ORDER BY CAST(s0.\"binary\" AS REAL)"
  end
  ...
  test "build_explain_query" do
    assert SQL.build_explain_query("SELECT 1", :query_plan) == "EXPLAIN QUERY PLAN SELECT 1"
  ...
  test "delete all with returning" do
    ...
    assert delete_all(query) == ~s{DELETE FROM "schema" AS s0 RETURNING "id", "x", "y", "z", "w", "meta"}
  end
  ...
  test "cross join with fragment" do
    query = from(p in Schema, cross_join: fragment("json_each(?)", p.j), select: {p.id}) |> plan()
    assert all(query) ==
  ...
  test "insert with on conflict" do
    ...
    assert query == ~s{INSERT INTO "schema" ("x","y") VALUES (?,?) ON CONFLICT ("x","y") DO NOTHING}
  ...
  test "insert with query" do
    ...
    assert_raise ArgumentError, fn ->
      insert(nil, "schema", [:x, :y, :z], [[:x, {query, 3}, :z], [nil, {query, 2}, :z]], {:raise, [], []}, [:id])
    end
  ...
  test "insert with query as rows" do
    ...
    query = from(s in "schema", select: %{ foo: fragment("3"), bar: s.bar }, where: true) |> plan(:all)
    query = insert(nil, "schema", [:foo, :bar], query, {:raise, [], []}, [:foo])
    assert query == ~s{INSERT INTO "schema" ("foo","bar") SELECT 3, s0."bar" FROM "schema" AS s0 WHERE (1) RETURNING "foo"}
  end
  ...
  test "update" do
    ...
    assert query == ~s{UPDATE "schema" SET "x" = ?, "y" = ? WHERE "id" = ? RETURNING "z"}
    ...
    assert query == ~s{UPDATE "prefix"."schema" SET "x" = ?, "y" = ? WHERE "id" = ? AND "updated_at" IS NULL}
  end
  ...
  test "create table with time columns" do
    ...
    assert execute_ddl(create) == [~s|CREATE TABLE "posts" ("published_at" TEXT, "submitted_at" TEXT)|]
  end
